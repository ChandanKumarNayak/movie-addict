...
class Instamojo {
    ...
    */
    ...
    /**
    * Uploads any file or cover image mentioned in $db and 
    * updates it with the json required by the API.
    * @param array $db
    * @return array $db updated with uploaded file information if applicable.
    */
    public function uploadMagic(array $db)
    {
        if(!empty($db['file_upload'])) {
            $file_upload_json = $this->uploadFile($db['file_upload']);
            $db['file_upload_json'] = $file_upload_json;
            unset($db['file_upload']);
        }
        if(!empty($db['cover_image'])) {
            $cover_image_json = $this->uploadFile($db['cover_image']);
            $db['cover_image_json'] = $cover_image_json;
            unset($db['cover_image']);
        }
        return $db;        
    }
    ...
    * @return array single Link object.
    */  
    public function linkCreate(array $db) 
    {   
        if(empty($db['currency'])){
            $db['currency'] = 'INR';
        }
        $db = $this->uploadMagic($db);
        $response = $this->api_call('POST', 'links', $db);
        return $response['link'];
    }
    ...
    * @return array single Link object.
    */  
    public function linkEdit($slug, array $db) 
    {
        $db = $this->uploadMagic($db);
        $response = $this->api_call('PATCH', 'links/' . $slug, $db);
        return $response['link'];
    }
